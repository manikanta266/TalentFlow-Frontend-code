...
<div className="flex flex-col md:flex-row w-full max-w-6xl space-y-8 md:space-y-0 md:space-x-8">
  ...
  <div className="grid grid-cols-4 gap-4 pt-3 w-auto">
    {workingWith.map((each) => (
      <div key={each.employeeId} className="bg-white p-3 rounded-lg shadow-md hover:bg-blue-50 cursor-pointer">
        <p className=" font-semibold text-blue-800" onClick={()=>handleAlsoWorkWith(each.employeeId)} >{each.firstName.charAt(0).toUpperCase() + each.firstName.slice(1).toLowerCase()} {each.lastName.charAt(0).toUpperCase() + each.lastName.slice(1).toLowerCase()}</p>
      </div>
    ))}
  </div>
</div>
