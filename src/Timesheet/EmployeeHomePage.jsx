...
const EmployeeHomePage = ({ submissions, setSubmissions }) => {
  ...
  const handleDeleteTimesheet = async () => {
    ...
    try {
      await axios.delete(
        `${url}/api/timesheets/delete/${selectedSubmissionId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "X-Tenant-ID":localStorage.getItem('company')
  ...
  const downloadTimesheet = (submission) => {
    ...
    doc.save(
      `Timesheet_${submission.clientName}_${submission.projectName}.pdf`
    );
  };

  const handleApplyDateRange = () => {
    if (startDate && endDate) {
      // Only apply the date range if both dates are provided
              ...
              <div className="flex flex-col md:flex-row gap-4">
                ...
                <div className="flex gap-4 mt-4 md:mt-0">
                  <button
                    ...
                    title={
                      !startDate || !endDate
                        ? "The button is disabled because the date range has not been applied yet."
                        : ""
                    }
                  >
                    Download
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {currentSubmissions.map((submission) => (
                      <tr key={submission.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-xl text-center font-medium text-gray-900">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-xl text-center font-medium text-gray-900">
                          {submission.numberOfHours}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-xl text-center font-medium text-gray-900">
                          {submission.extraHours}
                        </td>
                            ...
                            submission.status !== "REJECTED" && (
                              <div className="flex space-x-2 gap-4">
                                <button
                                  ...
                                  onClick={() =>
                                    handleEditTimesheet(submission)
                                  }
                                >
                                  <FaEdit />
                                </button>
            ...
            {isModalOpen && (
              <>
                ...
                <div className="fixed inset-0 flex items-center justify-center z-50">
                  <div>
                    <TimesheetManagement onClose={handleCloseModal} />{" "}
